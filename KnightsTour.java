...
public class KnightsTour {
    ...
    */
    ...
    public static void main(String[] args) {
        while(true) {
            ...
            if(userInput.equalsIgnoreCase("Done")) break; // if user inputs "done" --> end game
    ...
    private static void run() {
        ...
        resetNumberMoves();
        int moveNumber = 1;
        for (int i = 0; i < 63; i++) { // calculates moves 1 - 63
        ...
        printChessBoard(); // prints board and 64th move (only one possible move left)
        System.out.println();
    }
    /*
    ...
    private static void printChessBoard() {
        for (int i = 0; i < ChessBoard.length; i++) {
            for (int j = 0; j < ChessBoard[i].length; j++){
                if(ChessBoard[i][j] == 0) ChessBoard[i][j] = 64; // overwrites remaining square with 64
     ...
     */
    private static void markSpace(int count) {
        ChessBoard[rowIndex][columnIndex] = count;
    }
    /*
    ...
    */
    ...
    /*
    Prints game over. Ends the program.
     */
